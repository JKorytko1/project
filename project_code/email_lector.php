<?
	...
	for ($i = 0; $i < $count; $i++)
	{
		if($data[$i]['dead_line']<=date("Y-m-d", time() - 24 * 60 * 60))
		{
			...
			else
			{
				...
				$name[]="<b>Уважаемый преподаватель кафедры СПУ, ".$data[$i]['lector_name']
	...
	$k=0;
	$footer="В случае, если студент не допущен, не явился, не сдавал, то Вы должны поставить ему отрицательную оценку - единицу  ( 1 ).<br><br>
	<b>Выставите оценки, пожалуйста, в ближайшее время!</b><br><br>Это письмо отправлено автоматически, отвечать на него не нужно.<br>
	...
	if(!$emptyarr)
		...
		{
			mail ($newemail[$i], "Преподаватель", $newname[$i]."<br><br>".$newtext[$i], $header);
			$i++; 
		}  */
