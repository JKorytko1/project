<?
	...
	$query="SELECT st.student_email, st.student_name, gr.group_name, sub.subject_title, md.module_name, grm.dead_line 
			...
			WHERE st.student_email IS NOT NULL
			ORDER BY st.student_email, sub.subject_title, grm.dead_line";
	...
	for ($i = 0; $i < $count; $i++)
	{
		   ...
		   )
		{
			...
			if( $data[$i]['student_email']==$data[$i+1]['student_email']&&
			...
			else
			{
				...
				$name[]="<b>Уважаемый студент группы ".$data[$i]['group_name'].", ".$data[$i]['student_name']
				."!</b><br><br>Вы имеете задолженность по предметам:";
				$k++;
			}
	...
	$k=0;
	$footer="<b>Напоминаем Вам, что студент, который имеет отрицательную оценку (1, 2F) по предмету, 
	должен закрыть предмет в кратчайшие сроки, иначе студент может быть отчислен!</b>
	...
	if(!$emptyarr)
		...
		{
			mail ($newemail[$i], "Студент", $newname[$i]."<br><br>".$newtext[$i], $header);
			$i++; 
		}  
